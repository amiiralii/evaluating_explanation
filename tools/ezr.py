...
def trace(data, tree, row):
  "Print and return the decision path a row takes through the tree."
  trail = []
  lvl = 0
  while True:
    matched = None
    for kid in tree.kids:
      if treeSelects(row, *kid.how):
        matched = kid
        break
    ...
    # print('|  ')
    # print(f"{name} {op} {y}   (row {name}={row[at]})")
    trail.append((name, op, y, row[at], True))
    tree, lvl = matched, lvl + 1
  # print(f"n:{len(tree.rows):4}   mu:{tree.mu:.3f}")
  trail.append(("leaf", len(tree.rows), tree.mu))
  return trail

def bound_kind(op):
  "Translate a treeSelects op into a constraint direction."
  return {"==":"eq", "!=":"neq", "<=":"le", ">":"gt"}[op]

def summarize(data, trail):
  """Collapse a path's raw predicate sequence into one tightest constraint
  per feature: numeric -> ('num', lo, hi) interval, symbolic -> ('eq', v)
  or ('neq', frozenset) of excluded values."""
  out = {}
  for f, op, sv, rv, ok in trail[:-1]:
    kind = bound_kind(op)
    if kind == "eq":
      out[f] = ("eq", sv)
    elif kind == "neq":
      prev = out.get(f)
      if prev and prev[0] == "eq": continue
      excl = prev[1] if prev and prev[0] == "neq" else frozenset()
      out[f] = ("neq", excl | {sv})
    else:
      prev = out.get(f)
      lo, hi = (prev[1], prev[2]) if prev and prev[0] == "num" else (-big, big)
      lo, hi = (max(lo, sv), hi) if kind == "gt" else (lo, min(hi, sv))
      out[f] = ("num", lo, hi)
  return out

def region_score(data, ca, cb, f, tight=0.7):
  "Compatibility (0 / 0.5 / 1) between two paths' constraints on one shared feature."
  a, b = ca[f], cb[f]
  if a[0] == "eq" or b[0] == "eq":
    if a[0] == "eq" and b[0] == "eq":
      return 1.0 if a[1] == b[1] else 0.0
    eqv, other = (a, b) if a[0] == "eq" else (b, a)
    return 0.0 if other[0] == "neq" and eqv[1] in other[1] else 0.5
  if a[0] == "neq" and b[0] == "neq":
    return 1.0 if a[1] == b[1] else (0.5 if a[1] & b[1] else 0.0)
  at  = data.cols.names.index(f)
  col = data.cols.all[at]
  lo_a = 0.0 if a[1] == -big else norm(col, a[1])
  hi_a = 1.0 if a[2] ==  big else norm(col, a[2])
  lo_b = 0.0 if b[1] == -big else norm(col, b[1])
  hi_b = 1.0 if b[2] ==  big else norm(col, b[2])
  overlap = min(hi_a, hi_b) - max(lo_a, lo_b)
  if overlap <= 0: return 0.0
  union = max(hi_a, hi_b) - min(lo_a, lo_b)
  iou = overlap/union if union > 0 else 1.0
  return 1.0 if iou >= tight else 0.5

def region_similarity(data, trail_a, trail_b, tight=0.7):
  "Heuristic similarity between two paths' implied regions of feature space."
  ca, cb = summarize(data, trail_a), summarize(data, trail_b)
  common = set(ca) & set(cb)
  if not common: return 0.0
  score  = len(common)/len(ca) + len(common)/len(cb)
  score += sum(region_score(data, ca, cb, f, tight) for f in common)
  return score
